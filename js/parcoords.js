function render({ model, el }) {
    ...
    // Create and append button container with buttons
    ...
    const originalData = Object.freeze(model.get("data").map((d, idx) => ({ 
        ...
        _index: idx  // Store original index
    })));
    ...
    const gradientConfig = {
        startColor: "rgba(59, 130, 246, 1)",   // blue
        endColor: "rgba(217, 119, 6, 1)",      // orange
    ...
    // Check if we should use gradient coloring by checking if ALL color values are numbers
    const useGradient = uniqueGroups.length > 0 && 
                       originalData.every(d => typeof d.color === 'number');
    ...
    let isDraggingBrush = false; // True when dragging an existing brush
    let brushAxis = null; // Axis being interacted with (brushing or dragging)
    let brushStartY = null; // Initial Y for creating/resizing
    ...
    function getHighlightedData() {
        ...
        // Update the model with both the data and indices
        const selectionInfo = {
            data: highlighted,
    ...
    function handleMouseDown(event) {
        ...
        for (const dim in brushes) {
            if (brushes.hasOwnProperty(dim)) {
                ...
                if (pos.x >= minXRange && pos.x <= maxXRange && pos.y >= brushMinY && pos.y <= brushMaxY) {
                    ...
                    isBrushing = false; // Not creating/resizing
                    brushAxis = dim; // Axis whose brush is being dragged
                    draggedBrushInitialExtent = [...extent]; // Store initial state
                    dragStartOffsetY = pos.y - brushMinY; // Offset from top of brush rect
                    canvas.classList.add('dragging'); // Change cursor
                    // console.log(`Start dragging brush on ${dim}`);
                    break; // Stop checking other brushes
                }
            }
        }
        ...
        if (!clickedOnExistingBrush) {
            ...
            if (axis) {
                // Start creating/resizing brush
                const wasAlreadyBrushing = Object.keys(brushes).length > 0;
                isBrushing = true; // Creating/resizing mode
                ...
                // console.log(`Start brushing on ${axis}`);
                draw(); // Draw initial brush point/line
            } else {
                // 3. Clicked outside brushes and axis lines: Clear brushes
                if (Object.keys(brushes).length > 0) {
                    brushes = {};
                    isBrushing = false;
                    isDraggingBrush = false;
                    // console.log("Clearing brushes");
                    draw(); // Redraw (will trigger background update)
                    
                    // Update model with full dataset selection when brushes are cleared
                    model.set("selection", {
                        data: currentData,
    ...
    function handleMouseMove(event) {
        if (isDraggingBrush) {
            ...
            requestAnimationFrame(() => {
                if (!isDraggingBrush) return; // Check state again in case mouseup occurred
                const pos = getMousePos(canvas, event);
    ...
    function handleMouseUp(event) {
        ...
        if (isBrushing || isDraggingBrush) {
            ...
            // Get highlighted data and update model with both data and indices
    ...
    function handleReset() {
        ...
        // Update model with full dataset selection
        model.set("selection", {
            data: currentData,
            indices: currentData.map(d => d._index)
        });
